...
5. 对真实游戏样本运行 OCR，记录候选裁剪图、原始识别结果、置信度和词库匹配结果；Tesseract 结果只作基线参考。
6. 悬停符文让详情浮层遮挡底部按钮区域时，确认三张卡片仍展开、Overlay 不隐藏、OCR 不停止。
7. 后续 OCR 引擎验收至少覆盖多张真实样本、不同地图、画质和缩放；如果 Tesseract 继续低稳，切换本地 OCR 引擎适配层。
...
脚本会在 `ocr-debug-replay/` 下生成三槽 raw、focused、tight-line、enhanced 等 PNG 和 `replay-report.json`；如果离线 OCR 环境不可用，也会保留裁剪坐标和图片路径，便于人工查看定位。
